...
constexpr std::uint16_t kTestPort = 50064;
...
int main() {
  ...
  if (!WaitServerReady(std::chrono::seconds(2))) {
    ...
    std::cerr << "call_co_close_test failed: server not ready\n";
    return 1;
  }
  ...
  auto task = client.CallCo("CalcService", "Add", payload);
  client.Close();

  const auto result = rpc::coroutine::SyncWait(std::move(task));
  assert(!result.ok());
  ...
  std::cout << "call_co_close_test passed\n";
  return 0;
}
