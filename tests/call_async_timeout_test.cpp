...
constexpr std::uint16_t kTestPort = 50053;
...
int main() {
  ...
  if (!WaitServerReady(std::chrono::seconds(2))) {
    ...
    std::cerr << "call_async_timeout_test failed: server not ready\n";
    return 1;
  }
  ...
  auto future_fast1 = client.CallAsync("CalcService", "Add", fast1_payload);
  auto future_slow = client.CallAsync("CalcService", "Add", slow_payload);
  auto future_fast2 = client.CallAsync("CalcService", "Add", fast2_payload);

  const auto fast1_res = future_fast1.get();
  const auto fast2_res = future_fast2.get();
  const auto slow_res = future_slow.get();
  ...
  std::cout << "call_async_timeout_test passed\n";
  return 0;
}
